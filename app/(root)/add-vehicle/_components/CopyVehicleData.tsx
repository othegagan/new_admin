...
interface CopyVehicleDataProps {
    nextStep: () => void;
    previousStep: () => void;
}

export default function CopyVehicleData({ nextStep, previousStep }: CopyVehicleDataProps) {
    ...
    return (
        <div className='flex flex-col gap-4'>
            <CardTitle>{vehicleConfigTabsContent.import.title}</CardTitle>
            <CardDescription> {vehicleConfigTabsContent.import.description}</CardDescription>
            <Tabs defaultValue='bundee' className='mt-4'>
                ...
                <TabsContent value='turo'>
                    <div className='flex h-32 items-center justify-center'>Coming Soon</div>
                </TabsContent>
            </Tabs>

            <div className='mt-6 flex items-center justify-between gap-x-6'>
                <Button onClick={previousStep} variant='outline' prefix={<ArrowLeft className='size-4' />}>
                    Prev
                </Button>

                <Button type='submit' onClick={nextStep} variant='black' suffix={<ArrowRight className='size-4' />}>
                    Next
                </Button>
            </div>
        </div>
...
function CopyBundeeDataForm({ toVehicleId }: { toVehicleId: number }) {
    ...
    const allHostsVehicles =
        response?.data?.vehicleAndTripDetails?.sort((a: { make: string | null }, b: { make: string | null }) => {
    ...
    const completedVehicles =
        allHostsVehicles
            .filter((vehicle: any) => vehicle.uploadStatus === 'completed')
            .map((vehicle: any) => ({
