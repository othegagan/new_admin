        ...
        <div className='flex flex-col gap-4'>
            ...
            <div className='space-y-1'>
                {filteredLogs.map((log) => {
                    ...
                    if (!log.isTrip && !log.isVehicle && log.updateTag === 'PriceConstraint')
                        return <ConfigurationsLog key={log.id} log={log} user={user} />;
                })}
            </div>
        </div>
    );
...
function VehicleLogContent({ log }: { log: ActivityLog }) {
    ...
    return (
        <div className='mt-4 flex flex-col gap-5 md:flex-row md:px-20'>
            {displayData(oldData, 'old', log.updateTag)}
            <div className='w-px bg-neutral-400' />
            {displayData(newData, 'new', log.updateTag)}
        </div>
    );
}

function ConfigurationsLog({ log, user }: { log: ActivityLog; user: User }) {
    const oldData = log.from ? JSON.parse(log.from) : null;
    const newData = log.to ? JSON.parse(log.to) : null;
...
const displayData = (data: any, type: 'old' | 'new', updateTag: string) => {
