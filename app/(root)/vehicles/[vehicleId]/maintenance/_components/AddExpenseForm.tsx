...
import { env } from '@/env';
import { ExpenseIcon } from '@/public/icons';
...
const expenseTypes = ['Refueling', 'Parking', 'Tools', 'Registration', 'Finance', 'Other'];

type FormFields = z.infer<typeof expenseFormSchema>;

export default function AddExpenseForm({ vehicleId, refetchExpenseLogs }: { vehicleId: number; refetchExpenseLogs: any }) {
    ...
    const onSubmit: SubmitHandler<FormFields> = async (data) => {
        try {
            const { serviceType, date, time, driver, odometerReading, cost, paymentMethod, files, additionalNotes } = data;
            ...
            formData.append('spendDate', dateTime);
            ...
            const url = `${env.NEXT_PUBLIC_HOST_VEHICLE_SERVICES_BASEURL}/v1/vehicle/insertVehicleExpenseLog`;
    ...
    return (
        <>
            ...
            <AdaptiveDialog title='Add Expense' isOpen={isDialogOpen} onClose={closeDialog} className='sm:max-w-[70%]'>
                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4'>
                    <AdaptiveBody className='flex flex-col gap-5'>
                        <div className='grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3'>
                            <div className='group flex flex-col gap-1.5'>
                                <Label htmlFor='serviceType'>Expense Type</Label>
                                <Controller
                                    name='serviceType'
                                    control={control}
                                    render={({ field }) => (
                            ...
                            <div className='group flex flex-col gap-1.5'>
                                <Label htmlFor='driver'>
                                    Driver <span className='text-muted-foreground'>(Optional)</span>
                                </Label>
                                <Input {...register('driver')} />
                                {errors.driver && <FormError>{errors.driver.message}</FormError>}
                            </div>
                        </div>
