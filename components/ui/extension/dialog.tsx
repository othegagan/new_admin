...
const DialogContent = ({ className, children, side, role, closeButton = true, ...props }: DialogContentProps) => (
    <AriaModal
        ...
        {...props}>
        <AriaDialog role={role} className={cn(!side && 'grid h-full gap-4', 'h-full outline-hidden')}>
...
const DialogTitle = ({ className, ...props }: AriaHeadingProps) => (
    <AriaHeading slot='title' className={cn('font-semibold text-lg leading-none tracking-tight', className)} {...props} />
);
